...
`);
  ...
  var Rendered = class {
    ...
    cloneMerge(target, source, pruneMagicId) {
      let merged = __assign(__assign({}, target), source);
    ...
    toOutputBuffer(rendered, templates, output, changeTracking, rootAttrs = {}) {
      ...
      if (isRoot) {
        ...
        if (changeTracking || rendered.magicId) {
          ...
          attrs = __assign({[PHX_MAGIC_ID]: rendered.magicId}, rootAttrs);
        } else {
          attrs = rootAttrs;
        }
        if (skip) {
  ...
  var serializeForm = (form, metadata, onlyNames = []) => {
    const {submitter} = metadata, meta = __rest(metadata, ["submitter"]);
    let injectedElement;
  ...
  var View = class {
    ...
    pushInput(inputEl, targetCtx, forceCid, phxEvent, opts, callback) {
      ...
      if (inputEl.getAttribute(this.binding("change"))) {
        formData = serializeForm(inputEl.form, __assign({_target: opts._target}, meta), [inputEl.name]);
      } else {
        formData = serializeForm(inputEl.form, __assign({_target: opts._target}, meta));
      }
    ...
    pushFormSubmit(formEl, targetCtx, phxEvent, submitter, opts, onReply) {
      let refGenerator = () => this.disableForm(formEl, __assign(__assign({}, opts), {form: formEl, submitter}));
      ...
      if (LiveUploader.hasUploadsInProgress(formEl)) {
        ...
        this.uploadFiles(formEl, targetCtx, ref, cid, (uploads) => {
          ...
          let formData = serializeForm(formEl, __assign({submitter}, meta));
  ...
  var LiveSocket = class {
    ...
    bindTopLevelEvents({dead} = {}) {
      ...
      this.bind({keyup: "keyup", keydown: "keydown"}, (e, type, view, targetEl, phxEvent, phxTarget) => {
        ...
        let data = __assign({key: e.key}, this.eventMeta(type, e, targetEl));
